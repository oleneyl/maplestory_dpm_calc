...
class BasicVEnhancer(AbstractVEnhancer):
    ...
    def get_priority(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        return {
            "enhance": [
                {"name": skills[0].name.split("(")[0]}
                for skills in self.enhancer_priority
                if len(skills) > 0
            ],
            "vskill": [
                {"name": skills[0]["target"].name.split("(")[0]}
                for skills in v_skill_list_sorted
                if len(skills) > 0
            ],
        }
    ...
    def get_reinforcement_with_register(
        self, index: int, incr: int, crit: bool, target: AbstractSkill
    ) -> CharacterModifier:
        ...
        if index >= len(self.enhance_list):
            ...
            return VSkillModifier.get_reinforcement(
                incr, self.enhance_list[index], crit
            )
    ...
    def add_v_skill(
        self, target: AbstractSkill, use_index: int, upgrade_index: int
    ) -> None:
    ...
    def __repr__(self) -> str:
        return "VEnhancer :: dpmModule.jobs.template\nVEnhance : %s\nVSkill : %s" % (
            str(self.enhance_list),
            str(self.v_skill_list),
        )


class DirectVBuilder(AbstractVBuilder):
    def __init__(
        self, direct_enhance_state: List[int], direct_v_state: List[int]
    ) -> None:
        super().__init__()
...
class NjbStyleVBuilder(AbstractVBuilder):
    def __init__(
        self, skill_core_level: int = 25, each_enhanced_amount: int = 17
    ) -> None:
        super().__init__()
    ...
    def build_enhancer(
        ...
        return self.set_state_from_level_and_skill_cores(
            level, cores, self.skill_core_level, self.each_enhanced_amount
        )

    # TODO: each_enhanced_amount is not used.
    def set_state_from_level_and_skill_cores(
        self, level: int, skill_cores: int, skill_core_level: int, each_enhanced_amount
    ) -> BasicVEnhancer:
        ...
        while level_bonus > 0:
            ...
            while (
                ...
                if enhance_state_will_be_setted[target_upgrade_skill_index] < 60:
                    maximum_upgrade_level_available = (
                        60 - enhance_state_will_be_setted[target_upgrade_skill_index]
                    )
                    actual_chance_for_upgrade = min(
                        chance_for_upgrade, maximum_upgrade_level_available
                    )
                    enhance_state_will_be_setted[
                        target_upgrade_skill_index
                    ] += actual_chance_for_upgrade
                    enhancement_left -= 1
