...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        AdvancedRitualFanMastery = core.InformedCharacterModifier("고급 부채 숙련", att = 40 + combat, pdamage_indep = 35 + combat)
        Enlightenment = core.InformedCharacterModifier("득도", pdamage = 10 + combat)
        DragonsEye = core.InformedCharacterModifier("점정", att = 10 + combat, pdamage_indep = 10 + combat, crit = 10 + combat, crit_damage = 10 + combat, armor_ignore = 10 + combat)
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, combat : bool = False):
        ...
        # 1차
        #부채 타격 가정
        Humanity_As-You-Will_Fan = core.DamageSkill("여의선 : 인", 0, 525 + 5*combat, 5, modifier = core.CharacterModifier(pdamage_indep = 10)).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Talisman_Evil-Sealing_Gourd = core.DamageSkill("마봉 호로부", 0, 1000 + 10 * combat, 6, cooltime = -1, modifier = core.CharacterModifier(boss_pdamage = 20)).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)

        # 2차
        Earth_Ground-Shattering_Wave = core.DamageSkill("토파류 : 지", 0, 385 + 5*combat, 4).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Ground-Shattering_Wave_CT = core.DamageSkill("토파류 : 허/실", 0, 385 + 5*combat, 4).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        ...
        Talisman_Clone_Attack = core.DamageSkill("환영 분신부 (공격)", 0, 460 + 5*combat, 2 * 3 * TALISMAN_PROBABLITY, cooltime = 1500).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        ...
        Heaven_Iron_Fan_Gale = core.DamageSkill("파초풍 : 천", 0, 265 + 2*combat, 5).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Iron_Fan_Gale_CT = core.DamageSkill("파초풍 : 허/실", 0, 265 + 2*combat, 5).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)

        Earth_Stone_Tremor = core.DamageSkill("지진쇄 : 지", 0, 390 + 5*combat, 6, cooltime = 6000).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Stone_Tremor_CT = core.DamageSkill("지진쇄 : 허/실", 0, 390 + 5*combat, 6, cooltime = -1).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)

        Talisman_Seeking_Ghost_Flame = core.SummonSkill("추적 귀화부", 0, 1800, 390 + 5*combat, 5, 40*1000*SUMMON_REMAIN).setV(vEhc, 0, 0, False).wrap(core.SummonSkillWrapper)

        Scroll_Degeneration = core.DamageSkill("권술 : 미생강변", 0, 850 + 4*combat, 8, modifier = core.CharacterModifier(boss_pdamage = 20)).setV(vEhc, 0, 0, False).wrap(core.SummonSkillWrapper)
        Scroll_Degeneration_Debuff = core.BuffSkill("권술 : 미생강변 (디버프)", 0, 60*1000, armor_ignore = 20).wrap(core.BuffSkillWrapper)

        # 4차
        Heaven_Consuming_Flames = core.DamageSkill("멸화염 : 천", 0, 340 + combat, 6, cooltime = 8000).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Consuming_Flames_CT = core.DamageSkill("멸화염 : 허/실", 0, 340 + combat, 6).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)

        Humanity_Gold_Banded_Cudgel_1 = core.DamageSkill("금고봉 : 인 (1타)", 0, 260 + 3*combat, 10, cooltime = 11000, modifier = core.CharacterModifier(boss_pdamage = 30)).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Humanity_Gold_Banded_Cudgel_2 = core.DamageSkill("금고봉 : 인 (2타)", 0, 420 + combat, 8, modifier = core.CharacterModifier(boss_pdamage = 30)).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        
        Thousand_Ton_Stone = core.DamageSkill("둔갑 천근석", 0, 275 + 3*combat, 6, cooltime = 500).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        Thousand_Ton_Stone_DOT = core.DotSkill("둔갑 천근석 (출혈)", 270 + combat, 10000).setV(vEhc, 0, 0, False).wrap(core.SummonSkillWrapper)
        Thousand_Ton_Stone.onAfter(Thousand_Ton_Stone_DOT)

        Scroll_Star_Vortex = core.SummonSkill("권술 : 흡성와류", 0, 0, 240 + 4*combat, 6, 40000 * SUMMON_REMAIN).setV(vEhc, 0, 0, False).wrap(core.SummonSkillWrapper)

        # 벞지 & 소환수 지속시간 둘다 적용
        Scroll_Butterfly_Dream = core.BuffSkill("권술 : 호접지몽", 0, 100*1000, pdamage_indep = 10).wrap(core.BuffSkillWrapper)
        Scroll_Butterfly_Dream_Attack = core.DamageSkill("권술 : 호접지몽 (공격)", 0, 275 + 3 * combat, 5, cooltime = 1000).setV(vEhc, 0, 0, False).wrap(core.DamageSkillWrapper)
        ...
        # 5차 (베놈 버스트 생략)
        ReadyToDie = thieves.ReadyToDieWrapper(vEch, 0, 0)
