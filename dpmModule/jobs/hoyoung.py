...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        FiendSeal = core.InformedCharacterModifier("괴이봉인", patt = 10, final_damage = 10)
        RitualFanMastery = core.InformedCharacterModifier("부채 숙련", att = 25)
        ThirdEye = core.InformedCharacterModifier("심안", crit_rate = 30, crit_damage = 10)
        FortuneFitness = core.InformedCharacterModifier("신체 단련", stat_main = 60)
        Asura = core.InformedCharacterModifier("수라", att = 50, crit_rate = 10, crit_damage = 20, boss_pdamage = 20, armor_ignore = 10)
        AdvancedRitualFanMastery = core.InformedCharacterModifier("고급 부채 숙련", att = 40 + passive_level, final_damage = 35 + passive_level)
        Enlightenment = core.InformedCharacterModifier("득도", pdamage = 10 + ceil(passive_level/2))
        DragonsEye = core.InformedCharacterModifier("점정", att = 10 + passive_level, final_damage = 10 + passive_level, crit_rate = 10 + passive_level, crit_damage = 10 + passive_level, armor_ignore = 10 + passive_level)
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        #부채 타격 가정
        YeoUiSeon = core.DamageSkill("여의선 : 인", 540 + 30, 560 + 5*passive_level, 5, modifier = BASIC_HYPER + core.CharacterModifier(final_damage = 5)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)
        ...
        # 벞지 & 소환수 지속시간 둘다 적용
        Butterfly_Dream = core.BuffSkill("권술 : 호접지몽", 600, 100*1000, rem=True, final_damage = 10).wrap(core.BuffSkillWrapper)
