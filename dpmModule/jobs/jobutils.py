...
def get_weapon_att(chtr):
    return chtr.gear_list["weapon"].weapon_base_att
...
def restraint_ring(level: int = 4):
    return core.BuffSkill(
        "리스트레인트 링", 0, (7 + 2 * level) * 1000, cooltime=180000, patt=25 * level
    ).wrap(core.BuffSkillWrapper)


def risktaker_ring(level: int = 4):
    return core.BuffSkill(
        "리스트테이커 링", 0, (6 + 6 * level) * 1000, cooltime=180000, patt=(level + 1) * 10
    ).wrap(core.BuffSkillWrapper)


def crisis_ring(level: int = 4):
    return core.BuffSkill(
        "크라이시스 H/M 링", 0, (7 + 2 * level) * 1000, cooltime=180000, patt=25 * level
    ).wrap(core.BuffSkillWrapper)
...
def crit_damage_ring(level: int = 4):
    return core.BuffSkill(
        "크리데미지 링", 0, (7 + 2 * level) * 1000, cooltime=180000, crit_damage=7 * level
    ).wrap(core.BuffSkillWrapper)
...
def crit_defense_ring(level: int = 4):
    # 크확 100% 가정
    return core.BuffSkill(
        "크리디펜스 링", 0, (7 + 2 * level) * 1000, cooltime=180000, armor_ignore=25 * level
    ).wrap(core.BuffSkillWrapper)
