...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        ReincarnationBuff = core.InformedCharacterModifier(
            "리인카네이션(패시브)",
            pdamage_indep=30 + passive_level, crit=10 + ceil(passive_level / 3), crit_damage=15 + passive_level // 3
        )
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        DarkSpear = core.DamageSkill("다크 스피어", 750, 325+13*vEhc.getV(1, 0), 7*10, cooltime=10000, red=True, modifier=core.CharacterModifier(crit=100, armor_ignore=50)).isV(vEhc, 1, 0).wrap(core.DamageSkillWrapper)
        BiholderImpact = core.SummonSkill("비홀더 임팩트", 0, 270, 100+vEhc.getV(0, 2), 6, 2880, cooltime=20000, red=True, modifier=core.CharacterModifier(pdamage=150, armor_ignore=20)).setV(vEhc, 2, 3, False).isV(vEhc, 0, 2).wrap(core.SummonSkillWrapper)  # onTick으로 0.3초씩
