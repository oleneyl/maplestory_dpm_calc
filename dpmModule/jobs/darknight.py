...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        LordOfDarkness = core.InformedCharacterModifier("로드 오브 다크니스",crit_rate=30, crit_damage = 8)

        AdvancedWeaponMastery = core.InformedCharacterModifier("어드밴스드 웨폰 마스터리",att = 30 + passive_level, crit_damage = 15 + passive_level // 3)
        ReincarnationBuff = core.InformedCharacterModifier("리인카네이션(패시브)",final_damage = 30 + passive_level, crit_rate = 10 + ceil(passive_level / 3), crit_damage = 15 + passive_level // 3)
        ReincarnationHyper = core.InformedCharacterModifier("리인카네이션-크리티컬 레이트", crit_rate = 20)

        SacrificePassive = core.InformedCharacterModifier("새크리파이스(패시브)",armor_ignore = 30)
        CrossoverChainPassive = core.InformedCharacterModifier("크로스 오버 체인(패시브)", final_damage=50)
    ...
    def get_not_implied_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        BiholdersBuff = core.InformedCharacterModifier("비홀더스 버프",att = 40, crit_rate = 10)
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        CrossoverChain = core.BuffSkill("크로스 오버 체인", 0, 200*1000, final_damage = 20).wrap(core.BuffSkillWrapper) # 펫버프
        ...
        Reincarnation = core.BuffSkill("리인카네이션", 0, (40+passive_level)*1000, cooltime = (900 - 7 * passive_level) * 1000, rem = True, red = True, final_damage=30).wrap(core.BuffSkillWrapper) #궁그닐 쿨 무시
        ...
        DarkSpear = core.DamageSkill("다크 스피어", 750, 350+10*vEhc.getV(1,0), 7 * 8, cooltime = 10000, red = True, modifier = core.CharacterModifier(crit_rate=100, armor_ignore=50)).isV(vEhc,1,0).wrap(core.DamageSkillWrapper)
        ...
        PierceCycloneTick = core.DamageSkill("피어스 사이클론", 360, 400+16*vEhc.getV(3,3), 12, modifier = core.CharacterModifier(crit_rate=100, armor_ignore = 50)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper) #25타
        PierceCycloneEnd = core.DamageSkill("피어스 사이클론(종료)", 900, 300+12*vEhc.getV(3,3), 15*5, modifier = core.CharacterModifier(crit_rate=100, armor_ignore = 50)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper)
