...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        CrossoverChain = core.BuffSkill(DarkKnightSkills.CrossSurge, 0, 200*1000, pdamage_indep = 20).wrap(core.BuffSkillWrapper)  # Pet Buff. 펫버프.
        ...
        DarkSpear = core.DamageSkill(DarkKnightSkills.SpearofDarkness, 750, 350+10*vEhc.getV(1,0), 7*10, cooltime = 10000, red = True, modifier = core.CharacterModifier(crit=100, armor_ignore=50)).isV(vEhc,1,0).wrap(core.DamageSkillWrapper)
        BiholderImpact = core.SummonSkill(DarkKnightSkills.RadiantEvil, 0, 270, 100+vEhc.getV(0,2), 6, 2880, cooltime = 20000, red = True, modifier = core.CharacterModifier(pdamage = 150)).setV(vEhc, 2, 3, False).isV(vEhc,0,2).wrap(core.SummonSkillWrapper) # 0.3 seconds each with onTick. onTick으로 0.3초씩.
        ...
        DarknessAuraFinal = core.DamageSkill(_("{}(폭발)").format(DarkKnightSkills.DarknessAura), 0, 650+25*vEhc.getV(0,0), 13*(1+15//3), cooltime=-1).isV(vEhc,0,0).wrap(core.DamageSkillWrapper) # Adds 1 explosion every 3 health, maximum health 15. 생명력 3마다 폭발 1회 추가, 생명력 최대 15.
