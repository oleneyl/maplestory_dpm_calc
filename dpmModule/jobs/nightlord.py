...
class JobGenerator(ck.JobGenerator):
    ...
    def get_ruleset(self):
        ...
        ruleset.add_rule(
            ComplexConditionRule(
                "레디 투 다이", ["얼티밋 다크 사이트", "소울 컨트랙트"], ready_to_die_rule
            ),
            RuleSet.BASE,
        )
    ...
    def get_passive_skill_list(
        ...
        CriticalThrow = core.InformedCharacterModifier(
            "크리티컬 스로우", crit=50, crit_damage=5
        )
        PhisicalTraining = core.InformedCharacterModifier(
            "피지컬 트레이닝", stat_main=30, stat_sub=30
        )
        JavelinAccelation = core.InformedCharacterModifier(
            "자벨린 액셀레이션",
            stat_main=20,
        )

        SpiritJavelin = core.InformedCharacterModifier("스피릿 자벨린", att=10)
        Adrenalin = core.InformedCharacterModifier("아드레날린", crit_damage=10)
        JavelinMastery = core.InformedCharacterModifier(
            "숙련된 표창술", pdamage_indep=25, crit=20
        )
        PurgeAreaPassive = core.InformedCharacterModifier(
            "퍼지 에어리어(패시브)", boss_pdamage=10 + ceil(self.combat / 3)
        )
        DarkSerenity = core.InformedCharacterModifier(
            "다크 세레니티", att=40 + passive_level, armor_ignore=30 + passive_level
        )

        JavelineExpert = core.InformedCharacterModifier(
            "자벨린 엑스퍼트", att=30 + passive_level, crit_damage=15 + passive_level // 3
        )
    ...
    def generate(self, vEhc, chtr: ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        PurgeArea = core.BuffSkill(
            "퍼지 에어리어",
            delay=600,
            remain=(40 + self.combat) * 1000,
            armor_ignore=30 + self.combat,
        ).wrap(core.BuffSkillWrapper)
        BleedingToxin = core.BuffSkill(
            "블리딩 톡신",
            delay=780,
            remain=80 * 1000,
            cooltime=180 * 1000,
        ...
        BleedingToxinDot = core.DotSkill(
            "블리딩 톡신(도트)",
            summondelay=0,
            delay=1000,
            damage=1000,
        ...
        MarkOfNightlord = (
            core.DamageSkill(
                "마크 오브 나이트로드",
                delay=0,
                damage=(60 + 3 * passive_level + chtr.level),
                hit=MARK_PROP * 3,
                modifier=JAVELIN_ATT,
            )
            .setV(vEhc, 1, 2, True)
            .wrap(core.DamageSkillWrapper)
        )
        MarkOfNightlordPungma = (
            core.DamageSkill(
                "마크 오브 나이트로드(풍마)",
                delay=0,
                damage=(60 + 3 * passive_level + chtr.level),
            ...
            .setV(vEhc, 1, 2, True)
            .wrap(core.DamageSkillWrapper)
        )  # 툴팁대로면 19.355%가 맞으나, 쿼드러플과 동일한 37.5%로 적용되는 중

        FatalVenom = core.DotSkill(
            "페이탈 베놈",
            summondelay=0,
            delay=1000,
        ...
        ArcaneOfDarklord = (
            core.SummonSkill(
                ...
                summondelay=360,
                delay=1020,
                damage=350 + 14 * vEhc.getV(2, 2),
                hit=7 + 5,
                remain=11990,
                cooltime=60 * 1000,
                red=True,
                modifier=core.CharacterModifier(boss_pdamage=30) + JAVELIN_ATT,
            )
            .isV(vEhc, 2, 2)
            .wrap(core.SummonSkillWrapper)
        )  # 132타
        ...
        ThrowBlastingStack = core.StackSkillWrapper(
            core.BuffSkill("스로우 블래스팅(부적 스택)", 0, 99999999), 68
        )
        ThrowBlastingActive = (
            core.BuffSkill(
                "스로우 블래스팅(액티브)",
        ...
        ThrowBlastingPassive = (
            core.DamageSkill("스로우 블래스팅(패시브)", 0, 0, 0, cooltime=10000)
            .isV(vEhc, 0, 0)
            .wrap(core.DamageSkillWrapper)
        )
        ...
        QuarupleThrow.onAfter(
            core.OptionalElement(SpreadThrowInit.is_active, SpreadThrowTick)
        )
        QuarupleThrow.onAfter(
            core.OptionalElement(
                lambda: ThrowBlastingStack.judge(0, -1)
                and ThrowBlastingPassive.is_available(),
                ThrowBlastingPassive,
            )
        )
        QuarupleThrow.onAfter(
