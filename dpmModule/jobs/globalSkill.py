...
class GlobalSkills(Enum):
    TermsAndConditions = 'Terms and Conditions | 소울 컨트랙트'  # Taken from https://maplestory.fandom.com/wiki/Terms_and_Conditions
    MapleWarrior = 'Maple Warrior | 메이플 용사'  # Taken from https://maplestory.fandom.com/wiki/Maple_Warrior
    DecentCombatOrders = 'Decent Combat Orders | 쓸만한 컴뱃 오더스'  # Taken from https://maplestory.fandom.com/wiki/Decent_Combat_Orders
    DecentSharpEyes = 'Decent Sharp Eyes | 쓸만한 샤프 아이즈'  # Taken from https://maplestory.fandom.com/wiki/Decent_Sharp_Eyes
    DecentSpeedInfusion = 'Decent Speed Infusion | 쓸만한 윈드 부스터'  # Taken from https://maplestory.fandom.com/wiki/Decent_Speed_Infusion
    DecentHyperBody = 'Decent Hyper Body | 쓸만한 하이퍼 바디'  # Taken from https://maplestory.fandom.com/wiki/Decent_Hyper_Body
    DecentAdvancedBlessing = 'Decent Advanced Blessing'  # Taken from https://maplestory.fandom.com/wiki/Decent_Advanced_Blessing
    MapleWorldGoddessBlessing = 'Maple World Goddess\' Blessing | 메이플월드 여신의 축복'  # Taken from https://maplestory.fandom.com/wiki/Maple_World_Goddess%27s_Blessing
    TrueArachnidReflection = 'True Arachnid Reflection | 스파이더 인 미러'  # Taken from https://maplestory.fandom.com/wiki/True_Arachnid_Reflection
    TanadianRuin = 'Tanadian Ruin | 파괴의 얄다바오트'  # Taken from https://maplestory.fandom.com/wiki/Tanadian_Ruin_(Skill)
    AeonianRise = 'Aeonian Rise | 창조의 아이온'  # Taken from https://maplestory.fandom.com/wiki/Aeonian_Rise_(Skill)
    MitraFlame = 'Mitra Flame? | 크레스트 오브 더 솔라'  # Taken from
...
def useful_advanced_bless(slevel=1):
    return core.BuffSkill(GlobalSkills.DecentAdvancedBlessing.value, 600, usefulSkillRemain(slevel), rem=False, att=20).wrap(core.BuffSkillWrapper)
...
class TandadianRuinWrapper(core.BuffSkillWrapper):
    def __init__(self) -> None:
        skill = core.BuffSkill(GlobalSkills.TanadianRuin.value, 0, 30000, cooltime=90000, pdamage_indep=15, rem=False, red=False)
...
class MitraFlameWrapper(core.DamageSkillWrapper):
    def __init__(self, vEhc, num1, num2, modifier) -> None:
        skill = core.DamageSkill(f"{GlobalSkills.MitraFlame.value}(미트라의 불꽃)", 870, 750 + 30 * vEhc.getV(num1, num2), 12, cooltime=250 * 1000, red=True, modifier=modifier).isV(vEhc, num1, num2)
...
class FlamePatternWrapper(core.SummonSkillWrapper):
    def __init__(self, vEhc, num1, num2, modifier) -> None:
        skill = core.SummonSkill(f"{GlobalSkills.MitraFlame.value}(불꽃의 문양)", 0, 2100, 275 + 11 * vEhc.getV(num1, num2), 6, 51 * 1000, cooltime=-1, modifier=modifier).isV(vEhc, num1, num2)
        super(MirrorSpiderWrapper, self).__init__(skill)
...
def useful_hyper_body_demonavenger(slevel=1):
    ...
    return core.BuffSkill(f"{GlobalSkills.DecentHyperBody} (Demon Avenger | 데몬어벤져)", 600, usefulSkillRemain(slevel),
...
def useful_hyper_body_xenon(slevel=1):
    ...
    return core.BuffSkill(f"{GlobalSkills.DecentHyperBody} (Xenon | 제논)", 600, usefulSkillRemain(slevel),
