...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self):
        ...
        WindBlessingPassive = core.InformedCharacterModifier("윈드 블레싱(패시브)",pstat_main = 15, patt = 10 + self.combat)
        BowExpert = core.InformedCharacterModifier("보우 엑스퍼트", att = 30+self.combat, crit_damage = 20, pdamage_indep = 25+self.combat, boss_pdamage = 40+self.combat)
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, combat : bool = False):
        ...
        Albatross = core.BuffSkill("알바트로스 맥시멈", 0, 200 * 1000, att = 50+self.combat*1, pdamage = 25, armor_ignore = 15, crit = 25, rem = True).wrap(core.BuffSkillWrapper)  #900 -> 690
        SharpEyes = core.BuffSkill("샤프 아이즈", 660, 300 * 1000, crit = 20 + self.combat*1, crit_damage = 15 + self.combat*1, rem = True).wrap(core.BuffSkillWrapper)
        ...
        TriflingWhim = core.DamageSkill("트라이플링 윔", 0, (290+3*self.combat)*0.8+(390+3*self.combat)*0.2, 2*(0.5+0.1), modifier = core.CharacterModifier(pdamage = 20)).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)  #오더스 적용 필요
        ...
        #Damage Skills
        SongOfHeaven = core.DamageSkill("천공의 노래", 120, 345 + self.combat*3, 1, modifier = core.CharacterModifier(pdamage = 127.36, boss_pdamage = 30)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)#오더스 적용 필요, 코강렙 20이상 가정.
