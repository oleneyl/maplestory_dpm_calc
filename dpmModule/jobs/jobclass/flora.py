...
class FloraSkills(Enum):
    ...
    ConversionOverdrive = 'Conversion Overdrive | 매직 서킷 풀드라이브'  # Taken from https://maplestory.fandom.com/wiki/Conversion_Overdrive
...
def FloraGoddessBlessWrapper(vEhc, num1, num2, WEAPON_ATT):
    ...
    FloraGoddessBless = core.BuffSkill(f"{FloraSkills.GrandisGoddessBlessing.value}(Lef | 레프)", 480, 40*1000, att = 10 + 3 * vEhc.getV(num1, num2) + 1.5 * WEAPON_ATT, cooltime = 240*1000, red=True).isV(vEhc, num1, num2).wrap(core.BuffSkillWrapper)
