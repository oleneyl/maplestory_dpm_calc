...
1. 어빌 보공/크리/벞지 -> 보공/크리/상추뎀


2. 5차 강화 우선순위 변경: 죽음/증오 - 악몽/흉몽 - (플레인 - 공포/구속/고통) - (스칼렛/상처 - 거스트/굶주림 - 어비스/혼돈) - 충동/본능

    - 플레인 vs 공포/구속/고통이 좀 애매한데, 점유율이 허수아비의 경우 플레인이 좀 더 높지만
      실제로 거의 대부분의 보스전에서 공포/구속/고통이 더 높게 나타납니다. 일단 허수아비 기준으로 두었습니다.
    - 충동/본능이 레프 상태에서의 낮은 연계효율과 쿨타임 공유 문제로 우선순위가 가장 낮습니다.


3. 일부 스킬 수정

    3.1. 스칼렛 차지드라이브: 타수 수정
        - 이미 첫타와 후속타의 타수를 합쳐서 계산하셨던걸 못 보고 잘못된 정보로 제보드렸습니다... 죄송합니다ㅜㅜ

    3.2. 기어 다니는 공포: 딜레이 조정
        - 시작(LPFearPre)이 풀공속 시 690이나, 최소 30만 소요 가능
        - 종결(LPFearEnd): 630

    3.3. 끝없는 고통: 딜레이, 쿨타임 조정
        - 위와 같은 이유로 시작 딜레이 30 추가
        - 인게임 로직 상 키다운이 끝나고 나서야 쿨다운이 시작되기 때문에 실질적인 쿨타임은 약 63초

    3.4. 다가오는 죽음: 생성되는 개수를 2 → 1(인피상태 5 → 4)로 수정


4. 로직 변경

    4.1. 연계
        - [쿨없는 스킬] 플레인 차지드라이브(240ms)와 끝나지 않는 흉몽(180ms) 연계ver. 추가
        - [쿨있는 스킬] 스칼렛, 굶주림, 혼돈, 공포, 고통은 연계 시 딜레이 감소
        - [쿨있는 스킬]을 사용하기 전에는 딜레이가 감소된 [쿨없는 스킬]을 사용하도록 로직 변경

    4.2. 다가오는 죽음이 생성되는 스킬을 더 많이 연결
        - 다른 스펙터 스킬들과 함께 현재 인게임 로직에 맞게 매서풀(마력폭풍)도 추가

    4.3. 인피니티 스펠 상태 시 스펙터 모드에서 돌아오는 증오 생성개수를 12초에 12개로 제한

    4.4. 인피니티 스펠 상태 시 레프 모드에서 해당 스펠 + 플레인 스펠 2*4개 생성

    4.5. 어비스 스펠: 70뎀 2타로 13번 공격하는 SummonSkill로 변경
        - 지속시간이 10초가 아닌 3초입니다. 잘못된 정보로 제보드려 죄송합니다ㅠㅠ
        - 총 공격횟수 13번은 직접 테스트로 확인한 수치라서 여기에 맞게 주기를 끼워맞췄습니다.    

    4.6. 매직 서킷 풀드라이브(마력 폭풍): DamageSkill로 변경
        - 마력 폭풍이 실제로는 4초의 재발동 대기시간을 가지고 공격 스킬 적중 시 대상이 죽지 않았을 때에만 발동됩니다.

    4.7. 충동/본능 추가

    4.8. 딜싸이클 240s 주기에 최대한 맞추기
        - 스펙터 모드의 버프지속과 쿨타임을 억지로 조정 (근원 30s + 변신쿨 105s + 변신쿨 105s)
        - 근원, 인피, 매서풀 주기를 끼워맞추기 위한 룰 추가


        onBefore로 스킬을 지정하면 해당 스킬의 쿨타임을 무시하고 발동됩니다.
        그래프에 근원으로 늘어난 변신 유지 시간이 반영이 되지 않고 있습니다.
        버프1 또는 버프2가 발동 중일 때 스킬A를 발동한다는 조건은 어떻게 구현해야 할까
'''