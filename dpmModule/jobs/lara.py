...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        Exercise = core.InformedCharacterModifier("무구", pdamage=20, crit=20)
        WandExpert = core.InformedCharacterModifier("고급 지팡이 숙련", att=50+passive_level, pdamage_indep=30+passive_level//2)
        Insight = core.InformedCharacterModifier("혜안", crit=20+passive_level, crit_damage=20+passive_level, pdamage_indep=30+passive_level, armor_ignore=40+passive_level)
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        LaraAttack = core.DamageSkill("정기 뿌리기", 510, 250, 4).setV(vEhc, 0, 2).wrap(core.DamageSkillWrapper)
        MountainAtack = core.DamageSkill("산 꼬마", 0, 105+45+160+5*passive_level, 0.7*3, cooltime=-1).setV(vEhc, 0, 2).wrap(core.DamageSkillWrapper)

        ### 2차 스킬 ###
        Eruption = core.DamageSkill("용맥 분출", 0, 0, 0, 300).wrap(core.DamageSkillWrapper)  # TODO: 딜레이 확인
        Eruption_River = core.SummonSkill("분출 : 너울이는 강", 0, 2000, 450, 4, 18000).setV(vEhc, 0, 2).wrap(core.SummonSkillWrapper)
        Eruption_Wind = core.SummonSkill("분출 : 돌개바람", 0, 3200, 265, 5, 18000).setV(vEhc, 0, 2).wrap(core.SummonSkillWrapper)
        Eruption_Sun_Giant = core.DamageSkill("분출 : 해돋이 우물 (거인)", 0, 320, 6).setV(vEhc, 0, 2).wrap(core.DamageSkillWrapper)
        Eruption_Sun = core.SummonSkill("분출 : 해돋이 우물", 0, 1000, 220, 1, 18000).setV(vEhc, 0, 2).wrap(core.SummonSkillWrapper)
        Eruption_Sun_Bullet = core.DamageSkill("분출 : 해돋이 우물 (화산탄)", 0, 300, 3).setV(vEhc, 0, 2).wrap(core.DamageSkillWrapper)
        Eruption_Sun_Bullet_2 = core.DamageSkill("분출 : 해돋이 우물 (화산탄 다중히트)", 0, 300*0.9, 3*4).setV(vEhc, 0, 2).wrap(core.DamageSkillWrapper)
        Eruption_Sun_DOT = core.DotSkill("분출 : 해돋이 우물 (도트)", 0, 1000, 110, 1, 8000).setV(vEhc, 0, 2).wrap(core.SummonSkillWrapper)
        ...
        WakeUp_Add = core.DamageSkill("잠 깨우기(추가타)", 540, 105*0.6, 4*(6+9)/2, cooltime=-1).setV(vEhc, 0, 2).wrap(core.DamageSkillWrapper)  # 추가타: 6~9회
        ...
        Echo = core.BuffSkill("용맥의 메아리", 0, 20000, pdamage_indep=5, red=False).wrap(core.BuffSkillWrapper)
        ...
        HomeOfSpirits = core.BuffSkill("아름드리 나무", 660, 30000, 180*1000, armor_ignore=15, boss_pdamage=50, crit_damage=10).wrap(core.BuffSkillWrapper)
        ...
        BigEruption = core.DamageSkill("큰 기지개", 870, 400+16*vEhc.getV(0, 0), 5*1*5, 60000).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        BigEruption_Add = core.DamageSkill("큰 기지개 (추가타)", 0, (400+16*vEhc.getV(0, 0))*0.7, 5*(7-1)*5, -1).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        CombinationBlow = core.DamageSkill("해 강 산 바람", 840, 0, 0, 180*1000).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        CombinationBlow_Spirit = core.DamageSkill("해 강 산 바람 (토지령)", 0, 675+27*vEhc.getV(0, 0), 10*3, -1).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        CombinationBlow_Explosion = core.DamageSkill("해 강 산 바람 (폭발)", 0, 750+30*vEhc.getV(0, 0), 15*7, -1).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        ...
        BurstUp = core.SummonSkill("산등성이 굽이굽이", 960, 500, 325+13*vEhc.getV(0, 0), 3*4).isV(vEhc, 0, 0).wrap(core.SummonSkillWrapper)
