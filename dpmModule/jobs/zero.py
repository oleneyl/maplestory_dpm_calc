...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, combat : bool = False):
        ...
        # 베타: 패시브 스킬 차이만큼 적용
        # BetaSkill(x, y): x = 타겟 수, y = 코강으로 인한 타겟수 증가여부
        AlphaSkill = core.CharacterModifier(crit_damage = 20*4/35)
        BetaSkill = lambda x, y: (self.Beta - self.Alpha + core.CharacterModifier(pdamage = 8 * (x + int(y) - 1)))
        
        # 알파: 크리티컬 바인드 크뎀 평균값 적용
        AlphaState = core.BuffSkill("상태-알파", 0, 9999*10000, cooltime = -1).wrap(core.BuffSkillWrapper)
        BetaState = core.BuffSkill("상태-베타", 0, 9999*10000, cooltime = -1).wrap(core.BuffSkillWrapper)

        #### 알파 ####
        MoonStrike = core.DamageSkill("문 스트라이크", 390, 120, 6, modifier = AlphaSkill).setV(vEhc, 5, 3, False).wrap(core.DamageSkillWrapper)
        MoonStrikeTAG = core.DamageSkill("문 스트라이크(태그)", 0, 120, 6, modifier = AlphaSkill).setV(vEhc, 5, 3, False).wrap(core.DamageSkillWrapper)
        
        PierceStrike = core.DamageSkill("피어스 쓰러스트", 510, 170, 6, modifier = AlphaSkill).setV(vEhc, 0, 3, False).wrap(core.DamageSkillWrapper)
        PierceStrikeTAG = core.DamageSkill("피어스 쓰러스트(태그)", 0, 170, 6, modifier = AlphaSkill).setV(vEhc, 0, 3, False).wrap(core.DamageSkillWrapper)
        
        '''
        미사용 스킬

        ShadowStrike = core.DamageSkill("쉐도우 스트라이크", ?, 195, 8, modifier = AlphaSkill)
        ShadowStrikeAura = core.DamageSkill("쉐도우 스트라이크", 0, 310, 1, modifier = AlphaSkill)
        '''
        FlashAssault = core.DamageSkill("플래시 어썰터", 480, 165, 8, modifier = AlphaSkill).setV(vEhc, 6, 2, False).wrap(core.DamageSkillWrapper)
        FlashAssaultTAG = core.DamageSkill("플래시 어썰터(태그)", 0, 165, 8, modifier = AlphaSkill).setV(vEhc, 6, 2, False).wrap(core.DamageSkillWrapper)
        
        ### Dummy Skills
        _SpinCutter = core.DamageSkill("스핀 커터", 630, 260, 10, modifier = AlphaSkill).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper) 
        _RollingCurve = core.DamageSkill("롤링 커브", 960, 365, 12, modifier = AlphaSkill).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        _RollingAssulter = core.DamageSkill("롤링 어썰터", 960, 375, 12, modifier = AlphaSkill).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)
        _StormBreak = core.DamageSkill("스톰 브레이크", 690, 335, 10, modifier = AlphaSkill).setV(vEhc, 4, 2, False).wrap(core.DamageSkillWrapper)
        ### Dummy SKill End 

        AdvancedSpinCutter = core.DamageSkill("어드밴스드 스핀 커터", 630, 260+3*self._combat, 10, modifier = AlphaSkill).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedSpinCutterTAG = core.DamageSkill("어드밴스드 스핀 커터(태그)", 0, 260+3*self._combat, 10, modifier = AlphaSkill).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedSpinCutterAura = core.DamageSkill("어드밴스드 스핀 커터(오라)", 0, 130+3*self._combat, 4, modifier = AlphaSkill).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)
        
        AdvancedRollingCurve = core.DamageSkill("어드밴스드 롤링 커브", 960, 365+3*self._combat, 12, modifier = AlphaSkill).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedRollingCurveTAG = core.DamageSkill("어드밴스드 롤링 커브(태그)", 0, 365+3*self._combat, 12, modifier = AlphaSkill).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedRollingCurveAura = core.DamageSkill("어드밴스드 롤링 커브(오라)", 0, 350+self._combat, 2, modifier = AlphaSkill).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        
        AdvancedRollingAssulter = core.DamageSkill("어드밴스드 롤링 어썰터", 960, 375+2*self._combat, 12, modifier = AlphaSkill).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedRollingAssulterTAG = core.DamageSkill("어드밴스드 롤링 어썰터(태그)", 0, 375+2*self._combat, 12, modifier = AlphaSkill).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedRollingAssulterAura = core.DamageSkill("어드밴스드 롤링 어썰터(오라)", 0, 250+self._combat, 3, modifier = AlphaSkill).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)
        
        WindCutter = core.DamageSkill("윈드 커터", 540, 165, 8, modifier = AlphaSkill).setV(vEhc, 7, 2, False).wrap(core.DamageSkillWrapper)
        WindCutterSummon = core.SummonSkill("윈드 커터(소환)", 0, 500, 110, 3, 3000, cooltime=-1, modifier = AlphaSkill).setV(vEhc, 7, 2, False).wrap(core.SummonSkillWrapper)
        #WindCutterSummon = core.DamageSkill("윈드 커터(소환)", 0, 110, 3*3, modifier = AlphaSkill).setV(vEhc, 7, 2, False).wrap(core.DamageSkillWrapper) #3타 타격

        WindStrike = core.DamageSkill("윈드 스트라이크",600, 250, 8, modifier = AlphaSkill).setV(vEhc, 8, 2, False).wrap(core.DamageSkillWrapper)
        ...
        UpperStrike = core.DamageSkill("어퍼 슬래시", 690, 210, 6, modifier = BetaSkill(6, True)).setV(vEhc, 5, 3, False).wrap(core.DamageSkillWrapper)
        UpperStrikeTAG = core.DamageSkill("어퍼 슬래시(태그)", 0, 210, 6, modifier = BetaSkill(6, True)).setV(vEhc, 5, 3, False).wrap(core.DamageSkillWrapper)
        
        AirRiot = core.DamageSkill("어드밴스드 파워 스텀프", 570, 330 + 5*self._combat, 9, modifier = BetaSkill(6, True)).setV(vEhc, 0, 3, False).wrap(core.DamageSkillWrapper)
        AirRiotTAG = core.DamageSkill("어드밴스드 파워 스텀프(태그)", 0, 330 + 5*self._combat, 9, modifier = BetaSkill(6, True)).setV(vEhc, 0, 3, False).wrap(core.DamageSkillWrapper)
        AirRiotWave = core.DamageSkill("어드밴스드 파워 스텀프(파동)", 0, 330 + 5*self._combat, 9, modifier = BetaSkill(6, True)).setV(vEhc, 0, 3, False).wrap(core.DamageSkillWrapper)
        
        THROWINGHIT = 5
        FlashCut = core.DamageSkill("프론트 슬래시", 630, 205, 6, modifier = BetaSkill(6, True)).setV(vEhc, 6, 2, False).wrap(core.DamageSkillWrapper)
        ThrowingWeapon = core.SummonSkill("어드밴스드 스로잉 웨폰", 360, 300, 550 + 5*self._combat, 2, THROWINGHIT*300, cooltime=-1, modifier = BetaSkill(6, True)).setV(vEhc, 1, 2, False).wrap(core.SummonSkillWrapper)
        #ThrowingWeapon = core.DamageSkill("어드밴스드 스로잉 웨폰", 360, 550 + 5*self._combat, 2 * 5, modifier = BetaSkill(6, True)).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)    #5타 = 1.5s
        
        SpinDriver = core.DamageSkill("터닝 드라이브", 540, 260, 6, modifier = BetaSkill(6, True)).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedWheelWind = core.DamageSkill("어드밴스드 휠 윈드", 540, 200+2*self._combat, 2*7, modifier = BetaSkill(6, True)).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)     #   0.1초당 1타, 최대 7초, 7타로 적용
        
        GigaCrash = core.DamageSkill("기가 크래시", 630, 250, 6, modifier = BetaSkill(6, True)).setV(vEhc, 7, 2, False).wrap(core.DamageSkillWrapper)
        GigaCrashTAG = core.DamageSkill("기가 크래시(태그)", 0, 250, 6, modifier = BetaSkill(6, True)).setV(vEhc, 7, 2, False).wrap(core.DamageSkillWrapper)
        
        FallingStar = core.DamageSkill("점핑 크래시", 660, 225, 6, modifier = BetaSkill(6, True)).setV(vEhc, 8, 2, False).wrap(core.DamageSkillWrapper)
        # 충격파 값 포함
        FallingStarTAG = core.DamageSkill("점핑 크래시(태그)", 0, 225, 6, modifier = BetaSkill(6, True)).setV(vEhc, 8, 2, False).wrap(core.DamageSkillWrapper)
        FallingStarWave = core.DamageSkill("점핑 크래시(충격파)", 0, 225, 3, modifier = BetaSkill(6, True)).setV(vEhc, 8, 2, False).wrap(core.DamageSkillWrapper)
        
        AdvancedEarthBreak = core.DamageSkill("어드밴스드 어스 브레이크", 1170, 380+3*self._combat, 10, modifier = BetaSkill(6, True)).setV(vEhc, 4, 2, False).wrap(core.DamageSkillWrapper)
        AdvancedEarthBreakTAG = core.DamageSkill("어드밴스드 어스 브레이크(태그)", 0, 380+3*self._combat, 10, modifier = BetaSkill(6, True)).setV(vEhc, 4, 2, False).wrap(core.DamageSkillWrapper) 
        ...
        LimitBreakAttack = core.DamageSkill("리미트 브레이크", 0, 400+15*vEhc.getV(0,0), 5, modifier = BetaSkill(15, False)).isV(vEhc,0,0).wrap(core.DamageSkillWrapper)
        ...
        # 베타로 사용함.
        TwinBladeOfTime = core.DamageSkill("조인트 어택", 0, 0, 0, cooltime = 120*1000, modifier = BetaSkill(12, False)).wrap(core.DamageSkillWrapper)
        TwinBladeOfTime_1 = core.DamageSkill("조인트 어택(1)", 3480, 875+35*vEhc.getV(1,1), 8, modifier = BetaSkill(12, False)).wrap(core.DamageSkillWrapper)
        TwinBladeOfTime_2 = core.DamageSkill("조인트 어택(2)", 0, 835+33*vEhc.getV(1,1), 8, modifier = BetaSkill(12, False)).wrap(core.DamageSkillWrapper)
        TwinBladeOfTime_3 = core.DamageSkill("조인트 어택(3)", 0, 1000+40*vEhc.getV(1,1), 13, modifier = BetaSkill(12, False)).wrap(core.DamageSkillWrapper)
        # 45타수가 시스템상으로 잘 반영되는지 확인필요.
        TwinBladeOfTime_end = core.DamageSkill("조인트 어택(4)", 0, 900+36*vEhc.getV(1,1), 45, modifier = (BetaSkill(12, False) + core.CharacterModifier(armor_ignore = 100))).isV(vEhc,1,1).wrap(core.DamageSkillWrapper)
        
        #알파
        ShadowFlashAlpha = core.DamageSkill("쉐도우 플래시(알파)", 670, 500+20*vEhc.getV(2,2), 6, cooltime = 40*1000, red = True, modifier = AlphaSkill).isV(vEhc,2,2).wrap(core.DamageSkillWrapper)
        ShadowFlashAlphaEnd = core.DamageSkill("쉐도우 플래시(알파)(종료)", 0, 400+16*vEhc.getV(2,2), 15*3, modifier = AlphaSkill).isV(vEhc,2,2).wrap(core.DamageSkillWrapper)
        
        #베타
        ShadowFlashBeta = core.DamageSkill("쉐도우 플래시(베타)", 670, 600+24*vEhc.getV(2,2), 5, cooltime = 40*1000, modifier = BetaSkill(8, False), red = True).isV(vEhc,2,2).wrap(core.DamageSkillWrapper)
        ShadowFlashBetaEnd = core.DamageSkill("쉐도우 플래시(베타)(종료)", 0, 750+30*vEhc.getV(2,2), 12 * 2, modifier = BetaSkill(8, False)).isV(vEhc,2,2).wrap(core.DamageSkillWrapper)
