...
class DivineAuraWrapper(core.BuffSkillWrapper):
    def __init__(self, skill_list):
        skill = core.BuffSkill("디바인 스위프트/포스", 0, 99999999)  # Usually keeps the Divine Swift. 평소에는 디바인 스위프트 상태 유지.
...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        WindCutterSummon = core.DamageSkill("윈드 커터(소용돌이)", 0, 110, 3*2, cooltime=-1).setV(vEhc, 7, 2, False).wrap(core.DamageSkillWrapper)  # Lasts up to 3 seconds, hits twice. 최대 3초지속, 2회 타격.
        ...
        AdvancedStormBreakSummon = core.DamageSkill("어드밴스드 스톰 브레이크(소용돌이)", 0, 335+2*self.combat, 4, cooltime=-1).setV(vEhc, 4, 2, False).wrap(core.DamageSkillWrapper)  # Lasts up to 3 seconds, hits once. 최대 3초지속, 1회 타격.
        ...
        AdvancedPowerStompWaveTAG = core.DamageSkill("어드밴스드 파워 스텀프(파동)(태그)", 0, 330+5*self.combat, 9, modifier=extra_damage(6)+BetaMDF-AlphaMDF).setV(vEhc, 0, 3, False).wrap(core.DamageSkillWrapper)  # Beta stats are always applied. 항상 베타 스탯이 적용됨.
        ...
        AdvancedWheelWind = core.DamageSkill("어드밴스드 휠 윈드", 900, 200+2*self.combat, 2*7, modifier=extra_damage(6)).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)  # 1 stroke per 0.1 seconds, maximum 7 seconds, 7 strokes applied. 0.1초당 1타, 최대 7초, 7타로 적용.
        AdvancedWheelWindTAG = core.DamageSkill("어드밴스드 휠 윈드(태그)", 0, 200+2*self.combat, 2*7, modifier=extra_damage(6)).setV(vEhc, 3, 2, False).wrap(core.DamageSkillWrapper)  # 1 stroke per 0.1 seconds, maximum 7 seconds, 7 strokes applied. 0.1초당 1타, 최대 7초, 7타로 적용.
        ...
        TimeHolding = core.BuffSkill("타임 홀딩", 0, 90000, cooltime=180*1000, pdamage=10, red=True).wrap(core.BuffSkillWrapper)  # Reset cooldown. (Excluding Time Rewa / Limit Break). 쿨타임 초기화. (타임 리와 / 리미트 브레이크 제외).
