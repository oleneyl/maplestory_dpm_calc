...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        Assasinate1 = core.DamageSkill(_("{}(1타)").format(ShadowerSkills.Assassinate), 630, 275+4*self.combat, 6, modifier = core.CharacterModifier(pdamage=20, boss_pdamage = 20, armor_ignore = 10, pdamage_indep = STACK1RATE)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper, name=_("{} 1타").format(ShadowerSkills.Assassinate))   # ??. 쉐파.
        Assasinate2 = core.DamageSkill(_("{}(2타)").format(ShadowerSkills.Assassinate), 420, 350+5*self.combat, 6, modifier = core.CharacterModifier(pdamage=20, boss_pdamage = 20, armor_ignore = 10, pdamage_indep = STACK2RATE)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper, name=_("{} 2타").format(ShadowerSkills.Assassinate))   # ??. 쉐파.
        
        Assasinate1_D = core.DamageSkill(_("{}(1타)({})").format(ShadowerSkills.Assassinate, ShadowerSkills.DarkSight), 630, 275+4*self.combat, 6, modifier = core.CharacterModifier(pdamage=20+150+4*self.combat, boss_pdamage = 20, armor_ignore = 10, pdamage_indep = STACK1RATE)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper, name=_("{} 1타(닼사)").format(ShadowerSkills.Assassinate))   # ??. 쉐파.
        Assasinate2_D = core.DamageSkill(_("{}(2타)({})").format(ShadowerSkills.Assassinate, ShadowerSkills.DarkSight), 420, 350+5*self.combat, 6, modifier = core.CharacterModifier(pdamage=20+150+4*self.combat, boss_pdamage = 20, armor_ignore = 10, pdamage_indep = STACK2RATE)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper, name=_("{} 2타(닼사)").format(ShadowerSkills.Assassinate))   # ??. 쉐파.
        ...
        UltimateDarksight = core.BuffSkill(ThiefSkills.ShadowWalker, 750, 30000,
        ...
        SonicBlowTick = core.DamageSkill(_("{}(틱)").format(ShadowerSkills.SonicBlow), 107, 500+20*vEhc.getV(1,1), 7, modifier = core.CharacterModifier(armor_ignore = 100)).isV(vEhc,1,1).wrap(core.DamageSkillWrapper, name = _("{}(사용)").format(ShadowerSkills.SonicBlow)) # 7 * 15

        ShadowFormation = core.SummonSkill(ShadowerSkills.SlashShadowFormation, 0, 8000/12, 425+17*vEhc.getV(0,0), 8, 8000-1, cooltime=90000, red=True).isV(vEhc,0,0).wrap(core.SummonSkillWrapper)
        ShadowFormationFinal = core.DamageSkill(_("{}(우두머리)").format(ShadowerSkills.SlashShadowFormation), 0, 625+25*vEhc.getV(0,0), 15*4, cooltime=-1).isV(vEhc,0,0).wrap(core.DamageSkillWrapper)
