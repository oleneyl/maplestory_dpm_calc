...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(
        ...
        PrimaCriticalPassive = core.InformedCharacterModifier(
            "프리마 크리티컬(패시브)", stat_main=10, crit_damage=20
        )
        PrimaCritical = core.InformedCharacterModifier(
            "프리마 크리티컬", crit_damage=8.33
        )  # 스택식으로도 계산 가능. 150 / 18 = 8.33...

        BoomerangStepPassive = core.InformedCharacterModifier(
            "부메랑 스텝(패시브)", pdamage_indep=25 + 2 * (self.combat // 3)
        )

        ShadowerInstinctPassive = core.InformedCharacterModifier(
            "섀도어 인스팅트(패시브)", armor_ignore=20 + self.combat
        )
        ...
        return [
            ...
            SheildMastery,
            Grid,
            PrimaCriticalPassive,
            PrimaCritical,
            BoomerangStepPassive,
            ShadowerInstinctPassive,
            ReadyToDiePassive,
            DaggerExpert,
        ]
    ...
    def generate(self, vEhc, chtr: ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        # Buff skills
        Booster = core.BuffSkill("부스터", 0, 200 * 1000, rem=True).wrap(
            core.BuffSkillWrapper
        )
        FlipTheCoin = core.BuffSkill(
            "플립 더 코인", 0, 120 * 1000, pdamage=5 * 5, crit=10 * 5
        ).wrap(core.BuffSkillWrapper)
        ShadowerInstinct = core.BuffSkill(
            "섀도어 인스팅트",
            delay=900,
            remain=(200 + 6 * self.combat) * 1000,
            rem=True,
            att=40 + 30 + 2 * self.combat,
        ).wrap(core.BuffSkillWrapper)
        ...
        Assasinate1 = (
            core.DamageSkill(
                "암살(1타)",
                delay=630,
                damage=275 + 4 * self.combat,
                hit=6,
                modifier=core.CharacterModifier(
                    pdamage=20,
                    boss_pdamage=20,
                    armor_ignore=10,
                    pdamage_indep=STACK1RATE,
                ),
            )
            .setV(vEhc, 0, 2, False)
        ...
        Assasinate2 = (
            core.DamageSkill(
                "암살(2타)",
                delay=420,
                damage=350 + 5 * self.combat,
                hit=6,
                modifier=core.CharacterModifier(
                    pdamage=20,
                    boss_pdamage=20,
                    armor_ignore=10,
                    pdamage_indep=STACK2RATE,
                ),
            )
            .setV(vEhc, 0, 2, False)
            .wrap(core.DamageSkillWrapper)
        ...
        MesoExplosion = core.StackDamageSkillWrapper(
            core.DamageSkill("메소 익스플로전", delay=0, damage=120, hit=2).setV(
                vEhc, 2, 3, False
            ),
            MesoStack,
        ...
        UltimateDarksight = (
            core.BuffSkill(
                ...
                pdamage_indep=(100 + 10 + 5 + vEhc.getV(3, 3) // 5) / (100 + 5) * 100
        ...
        DarkSightTurnedOn = core.ConstraintElement(
            "다크사이트 여부 확인", AdvancedDarkSight, isNotDarkSight
        )

        MesoExplosion.onJustAfter(MesoStack.stackController(-20, name="메소 제거"))

        Assasinate2.onAfters(
            [
                SonicBlow.controller(1500 * STACK2RATE * 0.01, "reduce_cooltime"),
        ...
        Assasinate = core.OptionalElement(
            AdvancedDarkSight.is_active, Assasinate1_D, Assasinate1, name="닼사 여부"
        )
