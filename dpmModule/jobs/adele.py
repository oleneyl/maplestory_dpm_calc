...
class JobGenerator(ck.JobGenerator):
    ...
    def get_ruleset(self):
        ruleset = RuleSet()
        ruleset.add_rule(ReservationRule(_("{}(레프)").format(FloraSkills.GrandisGoddessBlessing), AdeleSkills.InfinityBlade), RuleSet.BASE)
        # ruleset.add_rule(ReservationRule(_("{}(버프)").format(FloraSkills.ConversionOverdrive), AdeleSkills.InfinityBlade), RuleSet.BASE)
        return ruleset
    ...
    def generate(self, vEhc, chtr: ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        Wonder = core.DamageSkill(_("{}(원더)").format(AdeleSkills.MagicDispatch), 0, 80 + 30 + 115 + 225 + passive_level * 3, 3 * 5, cooltime=8000).setV(vEhc, 6, 2, False).wrap(core.DamageSkillWrapper)  # Trigger skill hit every 8 seconds. 8초마다 트리거 스킬 적중시 시전.

        Ether = core.StackSkillWrapper(core.BuffSkill(AdeleSkills.AetherWeaving, 0, 9999999), 400)
        EtherTick = core.SummonSkill(_("{}(자연 회복)").format(AdeleSkills.AetherWeaving), 0, 10020, 0, 0, 9999999).wrap(core.SummonSkillWrapper)
        ...
        ResonanceStack = core.BuffSkill(_("{}(스택)").format(AdeleSkills.ResonanceRush), 0, 30 * 1000, cooltime=-1, pdamage_indep=10, armor_ignore=10).wrap(core.BuffSkillWrapper)  # Final damage 5, ?? 5, Max 2 times. Assume always overlapping. 최종뎀 5, 방무 5, 최대2회. 상시 중첩으로 가정.
        ...
        TerritoryEnd = core.DamageSkill(_("{}(종료)").format(AdeleSkills.ReignofDestruction), 0, 550 + 300 + passive_level * 5, 12, cooltime=-1).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        ...
        BlossomExceed = core.StackDamageSkillWrapper(core.DamageSkill(_("{}(초과)").format(AdeleSkills.AetherBloom), 0, 650 + self.combat * 6, 8, cooltime=-1, modifier=core.CharacterModifier(pdamage_indep=-25)).setV(vEhc, 3, 2, False),
            Order,
        ...
        Ruin = core.DamageSkill(_("{}(시전)").format(AdeleSkills.Ruin), 600, 0, 0, cooltime=60 * 1000, red=True).isV(vEhc, 2, 2).wrap(core.DamageSkillWrapper)  # Assumed to be cast in 4 seconds. 4초에 나누어서 시전되는 것으로 가정
        RuinFirstTick = core.SummonSkill(_("{}(소환)").format(AdeleSkills.Ruin), 0, 160, 250 + vEhc.getV(2, 2) * 10, 6, 2000, cooltime=-1).isV(vEhc, 2, 2).wrap(core.SummonSkillWrapper)  # 12 times, assumed to be used in 2 seconds. 12번, 2초에 나누어 사용으로 가정.
        RuinSecondTick = core.SummonSkill(_("{}(공격)").format(AdeleSkills.Ruin), 0, 250, 450 + vEhc.getV(2, 2) * 18, 9, 2000, cooltime=-1).isV(vEhc, 2, 2).wrap(core.SummonSkillWrapper)  # Assume that the use is divided into 8 times and 2 seconds. 8번, 2초에 나누어 사용으로 가정.
        ...
        RestoreTick = core.SummonSkill(_("{}(주기공격)").format(AdeleSkills.LegacyRestoration), 0, 2970, 900 + 36 * vEhc.getV(1, 1), 3, 30 * 1000, cooltime=-1).isV(vEhc, 1, 1).wrap(core.SummonSkillWrapper)  # Cast 11 times. 11회 시전
