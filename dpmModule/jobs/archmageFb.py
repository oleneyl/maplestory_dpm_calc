...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        MistEruption = core.DamageSkill("미스트 이럽션", 720, 45 + self.combat, 60, cooltime = 4 * 1000, red=False, modifier = core.CharacterModifier(armor_ignore = 40 + self.combat, pdamage_indep = ERUPTION_RATE[5]) + core.CharacterModifier(pdamage = 10, armor_ignore = 20)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)
        ...
        Explosion = core.DamageSkill("익스플로젼", 300 + 540, 405, 2).wrap(core.DamageSkillWrapper) # 선딜
