...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, chtr : ck.AbstractCharacter, combat : bool = False , vEhc = core.vEnhancer()):
        ...
        Panic_AuraWeapon = core.DamageSkill("패닉", 0, 1150 * (75 + vEhc.getV(3,2))*0.01, 1).wrap(core.DamageSkillWrapper)
        ...
        RaisingBlowInrage = core.DamageSkill("레이징 블로우", 600, 285, 4, modifier = core.CharacterModifier(pdamage = 20)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)  #이걸 사용함.
        RaisingBlowInrageFinalizer = core.DamageSkill("레이징 블로우", 0, 285, 2, modifier = core.CharacterModifier(pdamage = 20, crit = 100)).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)  #이걸 사용함. 둘을 연결해야 함.
        ...
        Insizing_AuraWeapon = core.DamageSkill("인사이징", 0, 576 * (75 + vEhc.getV(3,2))*0.01, 3).wrap(core.DamageSkillWrapper)    #870->640 오더스 적용 필요함. 도트뎀 (30초간 2초당 165%), 크리율에따른 뎀증 반영필요.
        InsizingBuff = core.BuffSkill("인사이징", 0, 30 * 1000, cooltime = -1, pdamage = 25).wrap(core.BuffSkillWrapper)
        ...
        ComboDesfort_AuraWeapon = core.DamageSkill("오라 웨폰", 0,(800 + 32*vEhc.getV(2,3)) * (75 + vEhc.getV(3,2))*0.01, 7).isV(vEhc, 2, 3).wrap(core.DamageSkillWrapper)
