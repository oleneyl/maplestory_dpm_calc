...
class JobGenerator(ck.JobGenerator):
    ...
    def get_ruleset(self):
        ruleset = RuleSet()
        ruleset.add_rule(InactiveRule(HeroSkills.Worldreaver.value, HeroSkills.InstinctualCombo.value, lang=self.lang), RuleSet.BASE)
        ruleset.add_rule(InactiveRule(HeroSkills.RisingRage.value, HeroSkills.InstinctualCombo.value, lang=self.lang), RuleSet.BASE)
        ruleset.add_rule(ReservationRule(GlobalSkills.MapleWorldGoddessBlessing.value, HeroSkills.InstinctualCombo.value, lang=self.lang), RuleSet.BASE)
        ruleset.add_rule(ConcurrentRunRule(GlobalSkills.TermsAndConditions.value, HeroSkills.BurningSoulBlade.value, lang=self.lang), RuleSet.BASE)
        return ruleset
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        ######   Skill   ######
        #Buff skills
        Fury = core.BuffSkill(HeroSkills.Rage.value, 0, 200*1000, att = 30, rem = True, lang=self.lang).wrap(core.BuffSkillWrapper)
        EpicAdventure = core.BuffSkill(HeroSkills.EpicAdventure.value, 0, 60*1000, cooltime = 120 * 1000, pdamage = 10, lang=self.lang).wrap(core.BuffSkillWrapper)
        
        #Damage Skills
        Panic = core.DamageSkill(HeroSkills.Panic.value, 720, 1150, 1, cooltime = 40000, lang=self.lang).setV(vEhc, 5, 3, False).wrap(core.DamageSkillWrapper)
        PanicBuff = core.BuffSkill(f"{HeroSkills.Panic.value}(Debuff | 디버프)", 0, 40000, cooltime = -1, pdamage_indep = 25, rem = False, lang=self.lang).wrap(core.BuffSkillWrapper)
        
        RagingBlow = core.DamageSkill(HeroSkills.RagingBlow.value, 600, 200 + 3*self.combat, 8, modifier = core.CharacterModifier(pdamage = 20), lang=self.lang).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)
        RagingBlowEnrage = core.DamageSkill(f"{HeroSkills.RagingBlow.value}(Enrage | 인레이지)", 600, 215+3*self.combat, 6, modifier = core.CharacterModifier(pdamage = 20), lang=self.lang).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)  # I use this. 이걸 사용함.
        RagingBlowEnrageFinalizer = core.DamageSkill(f"{HeroSkills.RagingBlow.value}(Enrage |인레이지)(Final blow | 최종타)", 0, 215+3*self.combat, 2, modifier = core.CharacterModifier(pdamage = 20, crit = 100), lang=self.lang).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)  # I use this. You need to connect the two. 이걸 사용함. 둘을 연결해야 함.
        
        Puncture = core.DamageSkill(HeroSkills.Puncture.value, 660, 576 + 7 * self.combat, 4, cooltime = 30 * 1000, lang=self.lang).setV(vEhc, 4, 2, False).wrap(core.DamageSkillWrapper)
        PunctureBuff = core.BuffSkill(f"{HeroSkills.Puncture.value}(Buff | 버프)", 0, (30 + self.combat // 2) * 1000, cooltime = -1, pdamage = 25 + ceil(self.combat / 2), lang=self.lang).wrap(core.BuffSkillWrapper)
        PunctureDot = core.DotSkill(f"{HeroSkills.Puncture.value}(Debuff | 도트)", 0, 2000, 165 + 3*self.combat, 1, (30 + self.combat // 2) * 1000, cooltime = -1, lang=self.lang).wrap(core.DotSkillWrapper)
    
        AdvancedFinalAttack = core.DamageSkill(HeroSkills.AdvancedFinalAttack.value, 0, 170 + 2*passive_level, 3 * 0.01 * (60 + ceil(passive_level/2) + 15), lang=self.lang).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)

        RisingRage = core.DamageSkill(HeroSkills.RisingRage.value, 750, 500, 8, cooltime = 10*1000, lang=self.lang).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)

        Valhalla = core.BuffSkill(HeroSkills.CryValhalla.value, 900, 30 * 1000, cooltime = 150 * 1000, crit = 30, att = 50, lang=self.lang).wrap(core.BuffSkillWrapper)

        MirrorBreak, MirrorSpider = globalSkill.SpiderInMirrorBuilder(vEhc, 0, 0, lang=self.lang)

        SwordOfBurningSoul = core.SummonSkill(HeroSkills.BurningSoulBlade.value, 810, 1000, (315+12*vEhc.getV(0,0)), 6, (60+vEhc.getV(0,0)//2) * 1000, cooltime = 120 * 1000, red=True, modifier = core.CharacterModifier(crit = 50), lang=self.lang).isV(vEhc, 0, 0).wrap(core.SummonSkillWrapper)
        
        ComboDeathFault = core.DamageSkill(HeroSkills.Worldreaver.value, 1260, 400 + 16*vEhc.getV(2,3), 14, cooltime = 20 * 1000, red=True, lang=self.lang).isV(vEhc, 2, 3).wrap(core.DamageSkillWrapper)
        ComboDeathFaultBuff = core.BuffSkill(f"{HeroSkills.Worldreaver.value}(Buff | 버프)", 0, 5 * 1000, rem = False, cooltime = -1, lang=self.lang).isV(vEhc, 2, 3).wrap(core.BuffSkillWrapper)
        
        ComboInstinct = core.BuffSkill(HeroSkills.InstinctualCombo.value, 360, 30 * 1000, cooltime = 240 * 1000, rem = False, red = True, lang=self.lang).isV(vEhc, 1, 1).wrap(core.BuffSkillWrapper)
        ComboInstinctFringe = core.DamageSkill(f"{HeroSkills.InstinctualCombo.value}(Tear | 균열)", 0, 200 + 8*vEhc.getV(1,1), 18, lang=self.lang).isV(vEhc, 1, 1).wrap(core.DamageSkillWrapper)
        ComboInstinctOff = core.BuffSkill(f"{HeroSkills.InstinctualCombo.value}(Exit | 종료)", 0, 1, cooltime = -1, lang=self.lang).wrap(core.BuffSkillWrapper)

        SwordIllusionInit = core.DamageSkill(f"{HeroSkills.SwordIllusion.value}(Cast | 시전)", 660, 0, 0, cooltime=30000, red=True, lang=self.lang).wrap(core.DamageSkillWrapper)
        SwordIllusion = core.DamageSkill(HeroSkills.SwordIllusion.value, 0, 125+5*vEhc.getV(0,0), 4, cooltime=-1, lang=self.lang).wrap(core.DamageSkillWrapper)
        SwordIllusionFinal = core.DamageSkill(f"{HeroSkills.SwordIllusion.value}(Final | 최종)", 0, 250+10*vEhc.getV(0,0), 5, cooltime=-1, lang=self.lang).wrap(core.DamageSkillWrapper)

        ######   Skill Wrapper   ######
        ComboAttack = ComboAttackWrapper(core.BuffSkill("Combo attack | 콤보어택", 0, 999999 * 1000, lang=self.lang), ComboDeathFaultBuff, vEhc, passive_level)
