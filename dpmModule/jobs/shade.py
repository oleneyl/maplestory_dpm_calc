...
class JobGenerator(ck.JobGenerator):
    ...
    def get_not_implied_skill_list(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        Weakness = core.InformedCharacterModifier("약화",pdamage = 20)  # It is debuff, but it is always activated. 디버프지만 상시발동가정.
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        ChainBombPunchTick = core.DamageSkill("파쇄 연권(키다운)", 930/8, 400+16*vEhc.getV(0,0), 5, cooltime=-1).isV(vEhc,0,0).wrap(core.DamageSkillWrapper)  # 8 times of 5 strokes, cast + key down 1320ms. 5타씩 8회, 시전+키다운 1320ms.
        ChainBombPunchFinal = core.DamageSkill("파쇄 연권(막타)", 810, 950+38*vEhc.getV(0,0), 15*3, cooltime=-1).isV(vEhc,0,0).wrap(core.DamageSkillWrapper)  # 3 times of 15 strokes. 15타씩 3회.
