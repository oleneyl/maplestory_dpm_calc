...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        #엘리시온 38타 / 3타
        Elision = core.BuffSkill("엘리시온", 750, 40 * 1000, cooltime = 180 * 1000, red=True).isV(vEhc,1,1).wrap(core.BuffSkillWrapper)    #시전딜레이 750ms
        ElisionBreak = core.SummonSkill("엘리시온(균열)", 0, (750 * 6 + 5000), 520 + 21*vEhc.getV(1,1), 5 * 6 * 2, (750 * 6 + 5000) * 5 - 1, cooltime=-1, modifier = FallingMoon).isV(vEhc,1,1).wrap(core.SummonSkillWrapper)    # 엘리시온 6회마다, 총 5회 발동
        ElisionStyx = core.DamageSkill("크로스 더 스틱스(엘리시온)", 750, 580/2, 5 * 5 * 2, modifier = FallingMoon).setV(vEhc, 2, 2, False).wrap(core.DamageSkillWrapper)
        
        #소울 이클립스
        SoulEclipse = core.SummonSkill("소울 이클립스", 270, 1000, 450 + 18 * vEhc.getV(3,3), 7, 40 * 1000, cooltime = 180 * 1000, red=True).isV(vEhc,3,3).wrap(core.SummonSkillWrapper)
