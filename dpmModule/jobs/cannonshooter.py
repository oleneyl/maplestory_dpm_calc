...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        MonkeyFuriousDot = core.DotSkill("몽키 퓨리어스(도트)", 0, 1000, 200, 1, 30000, cooltime = -1).wrap(core.DotSkillWrapper)
        OakRulet = core.BuffSkill("오크통 룰렛", 840, 180000, rem = True, cooltime = 180000, crit_damage = 1.25).wrap(core.BuffSkillWrapper)
        OakRuletDOT = core.DotSkill("오크통 룰렛(도트)", 0, 1000, 50, 1, 5000, cooltime = -1).wrap(core.DotSkillWrapper)
