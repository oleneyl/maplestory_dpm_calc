...
class JobGenerator(ck.JobGenerator):
    ...
    def generate(self, chtr : ck.AbstractCharacter, combat : bool = False , vEhc = core.vEnhancer()):
        ...
        Blast_AuraWeapon = core.DamageSkill("오라 웨폰", 0, 318 * (75 + vEhc.getV(2,2))*0.01, 8+2+1, modifier = core.CharacterModifier(crit=20, pdamage = 20)).wrap(core.DamageSkillWrapper)
        ...
        BlessedHammerActive = core.SummonSkill("블레스드 해머", 0, 600, (525+vEhc.getV(1,1)*21)*3-(250+vEhc.getV(1,1)*10)*2, 1, 30 * 1000, cooltime = 60 * 1000).isV(vEhc,1,1).wrap(core.SummonSkillWrapper)#딜레이 반영
        ...
        GrandCrossLargeTick = core.DamageSkill("그랜드 크로스", 800, 600 + vEhc.getV(3,3)*24, 43, modifier = core.CharacterModifier(crit = 100, crit_damage = 100)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper) #6s
        GrandCross = core.DamageSkill("그랜드 크로스 홀더", 480, 0, 0, cooltime = 150 * 1000).wrap(core.DamageSkillWrapper)#Loop = 480인걸로 봐서 재정의 필요할 수도 있음
        
        GrandCrossSmallTick_AuraWeapon = core.DamageSkill("오라 웨폰", 0, (350 + vEhc.getV(3,3)*14) * (75 + vEhc.getV(2,2))*0.01, 6, modifier = core.CharacterModifier(crit = 100, crit_damage = 100)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper) #6s
        GrandCrossLargeTick_AuraWeapon = core.DamageSkill("오라 웨폰", 0, (600 + vEhc.getV(3,3)*24) * (75 + vEhc.getV(2,2))*0.01, 6, modifier = core.CharacterModifier(crit = 100, crit_damage = 100)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper) #6s
