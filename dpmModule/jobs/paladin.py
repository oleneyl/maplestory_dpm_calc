...
class JobGenerator(ck.JobGenerator):
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        PaladinExpert = core.InformedCharacterModifier("팔라딘 엑스퍼트(두손둔기)",crit_damage = 5 + (32+passive_level) // 3, final_damage = 42+passive_level, crit_rate = 42+passive_level, armor_ignore = 15+ceil((32+passive_level)/2)) + core.ExtendedCharacterModifier(crit_damage= 5, armor_ignore = 10)
    ...
    def get_not_implied_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        Mastery = core.InformedCharacterModifier("숙련도",final_damage = -4.5+ 0.5*ceil(passive_level/2))    #오더스 기본적용!
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        ElementalForce = core.BuffSkill("엘리멘탈 포스", 0, 206 * 1000, final_damage = 21, rem = True).wrap(core.BuffSkillWrapper) # 펫버프

        EpicAdventure = core.BuffSkill("에픽 어드벤처", 0, 60*1000, cooltime = 120 * 1000, pdamage = 10).wrap(core.BuffSkillWrapper)

        HolyUnity = core.BuffSkill("홀리 유니티", 600, (40 + vEhc.getV(0,0)) * 1000, cooltime = 120 * 1000, red = True, final_damage = int(35 + 0.5*vEhc.getV(0,0))).isV(vEhc,0,0).wrap(core.BuffSkillWrapper)
        ...
        GrandCrossSmallTick = core.DamageSkill("그랜드 크로스(작은)", 200, 175 + vEhc.getV(3,3)*7, 12, modifier = core.CharacterModifier(crit_rate = 100, armor_ignore = 100)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper) #3s, 15*6타
        GrandCrossLargeTick = core.DamageSkill("그랜드 크로스(강화)", 146, 300 + vEhc.getV(3,3)*12, 12, modifier = core.CharacterModifier(crit_rate = 100, armor_ignore = 100)).isV(vEhc,3,3).wrap(core.DamageSkillWrapper) #6s, 41*6타
