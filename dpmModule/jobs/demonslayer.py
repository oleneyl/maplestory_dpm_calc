...
class JobGenerator(ck.JobGenerator):
    ...
    def get_modifier_optimization_hint(self):
        return core.CharacterModifier(crit_rate = 50, armor_ignore = 50, pdamage = 50)
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        Outrage = core.InformedCharacterModifier("아웃레이지",att = 50, crit_rate = 20)
        PhisicalTraining = core.InformedCharacterModifier("피지컬 트레이닝",stat_main = 30, stat_sub = 30)
        Concentration = core.InformedCharacterModifier("컨센트레이션",final_damage = 25)
    ...
    def get_not_implied_skill_list(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        EvilTorture = core.InformedCharacterModifier("이블 토쳐",final_damage = 15, crit_rate = 15) #상태이상에 걸렷을때만.
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter):
        ...
        DS_MODIFIER = core.CharacterModifier(pdamage = 30 + 30, final_damage = 43) - core.CharacterModifier(final_damage = 10)
        AW_MODIFIER = DS_MODIFIER + core.CharacterModifier(armor_ignore = 50, boss_pdamage = 50)

        DemonSlashRemainTime = core.BuffSkill("데몬 슬래시-리메인타임", 0, 4000, cooltime = -1, final_damage = 10).wrap(core.BuffSkillWrapper)
        ...
        DemonImpact = core.DamageSkill("데몬 임팩트", 660, 460 + 4 * self.combat, (6+1), modifier = core.CharacterModifier(crit_rate = 100, armor_ignore = 30 + ceil(self.combat / 3), boss_pdamage = 40 + self.combat, pdamage = 20)).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)
        ...
        DemonAwakning = core.BuffSkill("데몬 어웨이크닝", 1110, (35 + vEhc.getV(0,0))*1000, cooltime = 120 * 1000, red=True, crit_rate = (50 + int(0.5*vEhc.getV(0,0)))).isV(vEhc,0,0).wrap(core.BuffSkillWrapper)
        ...
        SpiritOfRage = core.SummonSkill("요르문간드", 810, 1080, (850+34*vEhc.getV(3,3)), 12, (10+int(0.2*vEhc.getV(3,3)))*1000, cooltime = (120 - int(0.5*vEhc.getV(3,3)))*1000, red=True, modifier = core.CharacterModifier(crit_rate = 100, armor_ignore = 50)).isV(vEhc,3,3).wrap(core.SummonSkillWrapper)
        SpiritOfRageEnd = core.DamageSkill("요르문간드(종료)", 0, 900+36*vEhc.getV(3,3), 15, cooltime = -1).isV(vEhc,3,3).wrap(core.DamageSkillWrapper)
        Orthros = core.SummonSkill("오르트로스(네메아)", 510, 2000, 400+16*vEhc.getV(1,1), 12, 40000, cooltime = 120*1000, red=True, modifier = core.CharacterModifier(crit_rate = 100, armor_ignore = 50)).isV(vEhc,1,1).wrap(core.SummonSkillWrapper)
        Orthros_ = core.SummonSkill("오르트로스(게리온)", 0, 3000, 900+36*vEhc.getV(1,1), 10, 40000, cooltime = -1, modifier = core.CharacterModifier(crit_rate = 100, armor_ignore = 50)).isV(vEhc,1,1).wrap(core.SummonSkillWrapper)

        DemonBaneInit = core.DamageSkill("데몬 베인(개시)", 240, 0, 0, cooltime=240*1000, red=True).isV(vEhc,0,0).wrap(core.DamageSkillWrapper)
        DemonBaneTick = core.DamageSkill("데몬 베인(1)", 3760/16, 325+13*vEhc.getV(0,0), 6, cooltime=-1, modifier=core.CharacterModifier(crit_rate=50, armor_ignore=30)).isV(vEhc,0,0).wrap(core.DamageSkillWrapper) # 3760ms 16회
        DemonBane2Pre = core.DamageSkill("데몬 베인(2)(선딜)", 600, 0, 0, cooltime=-1).isV(vEhc,0,0).wrap(core.DamageSkillWrapper)
        DemonBane2Tick = core.DamageSkill("데몬 베인(2)", 2400/21, 650+26*vEhc.getV(0,0), 7, cooltime=-1, modifier=core.CharacterModifier(crit_rate=50, armor_ignore=30)).isV(vEhc,0,0).wrap(core.DamageSkillWrapper) # 2400ms 21회
