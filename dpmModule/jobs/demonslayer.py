...
class JobGenerator(ck.JobGenerator):
    ...
    def get_ruleset(self):
        ...
        '''
        ruleset = RuleSet()
        ruleset.add_rule(InactiveRule(f'{DemonSlayerSkills.DemonLash.value}(1st hit | 1타)', DemonSlayerSkills.DemonAwakening.value), RuleSet.BASE)
        ruleset.add_rule(InactiveRule(f'{DemonSlayerSkills.DemonLash.value}(1st hit | 1타)', 'Demon Lash-remaining time | 데몬 슬래시-리메인타임'), RuleSet.BASE)
        ruleset.add_rule(InactiveRule(DemonSlayerSkills.CerberusChomp.value, DemonSlayerSkills.DemonAwakening.value), RuleSet.BASE)
        ruleset.add_rule(ConcurrentRunRule(DemonSlayerSkills.DemonCry.value, DemonSlayerSkills.DemonAwakening.value), RuleSet.BASE)
        ruleset.add_rule(InactiveRule(f'{DemonSlayerSkills.DemonBane.value}(Cast | 개시)', DemonSlayerSkills.DemonAwakening.value), RuleSet.BASE)
        return ruleset
    ...
    def get_passive_skill_list(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        DeathCurse = core.InformedCharacterModifier(DemonSlayerSkills.CurseofFury.value, pdamage=1)
        Outrage = core.InformedCharacterModifier(DemonSlayerSkills.Outrage.value, att=50, crit=20)
        PhisicalTraining = core.InformedCharacterModifier(DemonSlayerSkills.PhysicalTraining.value, stat_main=30, stat_sub=30)
        Concentration = core.InformedCharacterModifier(DemonSlayerSkills.FocusedFury.value, pdamage_indep=25)
        AdvancedWeaponMastery = core.InformedCharacterModifier(DemonSlayerSkills.BarricadeMastery.value, att=50+passive_level, crit_damage=15+passive_level//3)

        DarkBindPassive = core.InformedCharacterModifier(f"{DemonSlayerSkills.BindingDarkness.value}(Passive | 패시브)", armor_ignore=30+self.combat)
    ...
    def generate(self, vEhc, chtr : ck.AbstractCharacter, options: Dict[str, Any]):
        ...
        DemonSlashRemainTime = core.BuffSkill("Demon Lash-remaining time | 데몬 슬래시-리메인타임", 0, 4000, cooltime=-1, pdamage_indep=10).wrap(core.BuffSkillWrapper)

        # When using only 1 stroke, the delay is longer than when it leads to 2 strokes. 1타만 사용시 딜레이가 2타로 이어질때보다 김.
        DemonSlash1 = core.DamageSkill(f"{DemonSlayerSkills.DemonLash.value}(1st hit | 1타)", 390, 110+80, 2, modifier=DS_MODIFIER).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)

        DemonSlashAW1 = core.DamageSkill("Demon Lash Awakening | 데몬 슬래시 강화(1st hit | 1타)", 270, 600+80, 3, modifier=AW_MODIFIER).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)
        DemonSlashAW2 = core.DamageSkill("Demon Lash Awakening | 데몬 슬래시 강화(2nd hit | 2타)", 270, 600+80, 3, modifier=AW_MODIFIER).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)
        DemonSlashAW3 = core.DamageSkill("Demon Lash Awakening | 데몬 슬래시 강화(3rd hit | 3타)", 360, 700+80, 3, modifier=AW_MODIFIER).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)
        DemonSlashAW4 = core.DamageSkill("Demon Lash Awakening | 데몬 슬래시 강화(4th hit | 4타)", 360, 800+80, 3, modifier=AW_MODIFIER).setV(vEhc, 0, 2, False).wrap(core.DamageSkillWrapper)

        DemonImpact = core.DamageSkill(DemonSlayerSkills.DemonImpact.value, 660, 460+4*self.combat, 6+1, modifier=core.CharacterModifier(crit=100, armor_ignore=30+ceil(self.combat/3), boss_pdamage=40+self.combat, pdamage=20)).setV(vEhc, 1, 2, False).wrap(core.DamageSkillWrapper)

        DevilCry = core.DamageSkill(DemonSlayerSkills.DemonCry.value, 1260, 515+5*self.combat, 7, cooltime=20*1000).setV(vEhc, 5, 2, False).wrap(core.DamageSkillWrapper)   # Required to use for Evil Torch. 이블 토쳐 위해 사용필수.
        DevilCryBuff = core.BuffSkill(f"{DemonSlayerSkills.DemonCry.value}(Threat | 위협)", 0, 20000, cooltime=-1, armor_ignore=15+self.combat//3).wrap(core.BuffSkillWrapper)

        InfinityForce = core.BuffSkill(DemonSlayerSkills.BoundlessRage.value, 990, (50+10*(self.combat//5))*1000, cooltime=(200-self.combat)*1000, rem=True, red=True).wrap(core.BuffSkillWrapper)
        Metamorphosis = core.BuffSkill(DemonSlayerSkills.DarkMetamorphosis.value, 1680, (180+4*self.combat)*1000, rem=True, pdamage=35+self.combat).wrap(core.BuffSkillWrapper)
        MetamorphosisSummon = core.SummonSkill(f"{DemonSlayerSkills.DarkMetamorphosis.value}(Summon | 소환)", 0, 510, 250+5*self.combat, 1, (180+4*self.combat)*(1+buff_rem/100)*1000, cooltime=-1).setV(vEhc, 4, 2, False).wrap(core.SummonSkillWrapper)
        MetamorphosisSummon_BB = core.DamageSkill(f"{DemonSlayerSkills.DarkMetamorphosis.value}(Blue Blood | 블블)", 0, (250+5*self.combat)*0.9, 1, cooltime=-1).setV(vEhc, 4, 2, False).wrap(core.DamageSkillWrapper)
        ...
        DemonAwakning = core.BuffSkill(DemonSlayerSkills.DemonAwakening.value, 1110, (35+vEhc.getV(0, 0))*1000, cooltime=120*1000, red=True, crit=(50+vEhc.getV(0, 0)//2)).isV(vEhc, 0, 0).wrap(core.BuffSkillWrapper)
        DemonAwakningSummon = core.SummonSkill(f"{DemonSlayerSkills.DemonAwakening.value}(Summon | 더미)", 0, 8000, 0, 0, (35+vEhc.getV(0, 0))*1000, cooltime=-1).isV(vEhc, 0, 0).wrap(core.SummonSkillWrapper)

        SpiritOfRage = core.SummonSkill(DemonSlayerSkills.SpiritofRage.value, 810, 1080, (850+34*vEhc.getV(3, 3)), 12, (10+vEhc.getV(3, 3)//5)*1000, cooltime=(120-vEhc.getV(3, 3)//2)*1000, red=True, modifier=core.CharacterModifier(crit=100, armor_ignore=50)).isV(vEhc, 3, 3).wrap(core.SummonSkillWrapper)
        SpiritOfRageEnd = core.DamageSkill(f"{DemonSlayerSkills.SpiritofRage.value}(Ending | 종료)", 0, 900+36*vEhc.getV(3, 3), 15, cooltime=-1).isV(vEhc, 3, 3).wrap(core.DamageSkillWrapper)
        Orthros = core.SummonSkill(f"{DemonSlayerSkills.Orthrus.value}(Nemea | 네메아)", 510, 2000, 400+16*vEhc.getV(1, 1), 12, 40000, cooltime=120*1000, red=True, modifier=core.CharacterModifier(crit=100, armor_ignore=50)).isV(vEhc, 1, 1).wrap(core.SummonSkillWrapper)
        Orthros_ = core.SummonSkill(f"{DemonSlayerSkills.Orthrus.value}(Geryon | 게리온)", 0, 3000, 900+36*vEhc.getV(1, 1), 10, 40000, cooltime=-1, modifier=core.CharacterModifier(crit=100, armor_ignore=50)).isV(vEhc, 1, 1).wrap(core.SummonSkillWrapper)

        DemonBaneInit = core.DamageSkill(f"{DemonSlayerSkills.DemonBane.value}(Cast | 개시)", 240, 0, 0, cooltime=240*1000, red=True).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        DemonBaneTick = core.DamageSkill(f"{DemonSlayerSkills.DemonBane.value}(1)", 3760/16, 325+13*vEhc.getV(0, 0), 6, cooltime=-1, modifier=core.CharacterModifier(crit=50, armor_ignore=30)).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)  # 3760ms 16 times. 3760ms 16회.
        DemonBane2Pre = core.DamageSkill(f"{DemonSlayerSkills.DemonBane.value}(2)(?? | 선딜)", 600, 0, 0, cooltime=-1).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        DemonBane2Tick = core.DamageSkill(f"{DemonSlayerSkills.DemonBane.value}(2)", 2400/21, 650+26*vEhc.getV(0, 0), 7, cooltime=-1, modifier=core.CharacterModifier(crit=50, armor_ignore=30)).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)  # 2400 ms 21 times. 2400ms 21회.
        DemonBane2After = core.DamageSkill(f"{DemonSlayerSkills.DemonBane.value}(2)(?? | 후딜)", 240, 0, 0, cooltime=-1).isV(vEhc, 0, 0).wrap(core.DamageSkillWrapper)
        ...
        BasicAttack = core.OptionalElement(DemonAwakning.is_active, DemonSlashAW1, DemonImpact, name="Awakening ON | 어웨이크닝 ON")
